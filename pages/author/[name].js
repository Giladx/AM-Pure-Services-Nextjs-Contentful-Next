...
import authorPageInitialPathsE43f7Resource from '../../resources/author-page-initial-paths-e43f7'
import authorPageInitialProps6ac32Resource from '../../resources/author-page-initial-props-6ac32'
...
export async function getStaticPaths() {
  try {
    const response = await authorPageInitialPathsE43f7Resource({
      content_type: 'author',
      select: 'fields.name',
    })
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorPageInitialProps6ac32Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {

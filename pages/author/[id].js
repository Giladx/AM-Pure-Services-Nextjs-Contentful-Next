...
import authorPageInitialPathsFd486Resource from '../../resources/author-page-initial-paths-fd486'
import authorPageInitialProps25130Resource from '../../resources/author-page-initial-props-25130'
...
export async function getStaticPaths() {
  try {
    const response = await authorPageInitialPathsFd486Resource({
      content_type: 'author',
      select: 'sys.id',
    })
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorPageInitialProps25130Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
